...
class RiviereGraphique(object):
    ...
    def __init__(self, jeu, titre='La rivière de l''IUT''O', size=(1000, 800), couleur=(209, 238, 238),
                 prefixeImage="./images"):
        pygame.init()
        self.jeu = jeu
    ...
    def polygone(self, Surface, color, n, radius, position):
        ...
        pygame.draw.lines(Surface, color, True,
                          [(math.cos(i / n * pi2) * radius + position[0], math.sin(i / n * pi2) * radius + position[1])
                           for i in range(0, n)])
    ...
    def miseAJourParametres(self):
        ...
        self.surfFond = pygame.transform.smoothscale(self.images["fond"], (
            (self.rayon + self.rayon // 2) * self.nbCol + self.cote, self.hauteur * self.nbLig // 2 + self.hauteur))
        ...
        for j in self.imgJoueurs:
            self.surfJoueur[j] = pygame.transform.smoothscale(self.imgJoueurs[j],
                                                              (int(self.cote * 1.2), int(self.cote * 1.3)))
    ...
    def getImages(self, prefixImage="./images"):
        ...
        for nomJ in self.nomsPossibles:
            self.imgJoueurs[getRepresentationJoueur(self.possibles, nomJ)] = self.chargeImage(nomJ + ".png",
                                                                                              prefixImage)
    ...
    def dessinerGrille(self, msg='', mode=0):
        ...
        self.surface.blit(self.surfDepArr, (
            debutV - self.cote // 2 + self.colDepart * (self.rayon // 2 + self.cote),
            debutH // 2 - self.hauteur // 6))
        self.surface.blit(self.surfDepArr, (debutV - self.cote // 2 + self.colArrivee * (self.rayon // 2 + self.cote),
                                            debutH - 2 * self.rayon - self.hauteur // 6 + self.nbLig * self.hauteur // 2))
        if not self.paire:
            debutH += self.cote / 2 + self.rayon
            cDebut = 1
        for i in range(self.nbLig):
            for j in range(cDebut, self.nbCol, 2):
                self.polygone(self.surface, (0, 0, 0), 6, self.rayon,
                              (debutH + (j // 2) * (2 * self.rayon + self.cote), debutV + i * (self.hauteur // 2)))
                contenu = getContenuR(self.riviere, i, j)
                if contenu == ROCHER:
                    self.surface.blit(self.surfRocher, (
                        debutH + (j // 2) * (2 * self.rayon + self.cote) - (self.rayon - 1),
                        debutV + i * (self.hauteur // 2) - (self.hauteur // 2 - 1)))
                else:
                    self.surface.blit(self.surfEau, (
                        debutH + (j // 2) * (2 * self.rayon + self.cote) - (self.rayon - 1),
                        debutV + i * (self.hauteur // 2) - (self.hauteur // 2 - 1)))
                courant = getCourantR(self.riviere, i, j)
                f = self.flecheCourant(courant, self.surfFleche)
                if f is not None:
                    self.surface.blit(f, (debutH + (j // 2) * (2 * self.rayon + self.cote) - (self.cote * 1.5 // 2),
                                          debutV + i * (self.hauteur // 2) - (self.cote // 2)))
                if contenu == TRONC:
                    self.surface.blit(self.surfTronc, (
                        debutH + (j // 2) * (2 * self.rayon + self.cote) - (self.cote * 1.2 // 2),
                        debutV + i * (self.hauteur // 2) - (self.cote * 1.2 // 2)))
                elif contenu not in (VIDE, ROCHER):
                    self.surface.blit(self.surfJoueur[contenu], (
                        debutH + (j // 2) * (2 * self.rayon + self.cote) - (self.cote // 2),
                        debutV + i * (self.hauteur // 2) - (self.cote * 1.2 // 2)))
            if cDebut == 0:
                cDebut = 1
                debutH += self.cote / 2 + self.rayon
    ...
    def afficherMsgJeu(self, msg):
        if getNbJoueursJ(self.jeu) != 0:
            self.afficherMessage(1, "C'est au tour du joueur @img@",
                                 [self.surfJoueur[getRepresentation(getJoueurCourantJ(self.jeu))]])
    ...
    def getCase(self, pos):
        ...
        if self.paire and h % 2 == 0 or not self.paire and h % 2 == 1:
            ...
            if num == 1:
                ...
                b1 = pos[1] - self.cotanPi6 * pos[0]
                b2 = (h / 2 * self.hauteur + self.debutV - self.hauteur / 2) - self.cotanPi6 * (
                    (d - auxD) * self.rayon + self.debutH)
                col += auxCol
                if b1 < b2:
                    lig = h - 1
                else:
        ...
        while True:
            ...
            if ev.type == pygame.KEYDOWN:
                if self.mode == 0:
                    ...
                    if ev.key not in [pygame.K_NUMLOCK, pygame.K_CAPSLOCK, pygame.K_SCROLLOCK, pygame.K_RSHIFT,
                                      pygame.K_LSHIFT, pygame.K_RCTRL, pygame.K_LCTRL, pygame.K_RALT, pygame.K_LALT,
                                      pygame.K_RMETA, pygame.K_LMETA, pygame.K_LSUPER, pygame.K_RSUPER, pygame.K_MODE]:
